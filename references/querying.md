...
- The sizing entities are deterministic and precomputed for `recommended` and `active_only`, using both `consolidated` and `source_aligned` topologies. Filter all three dimensions explicitly. Use `recommended` unless the user deliberately selects active-only sizing. The precomputed views use provisioned storage without automatic growth. Do not add another N+1 host or apply new headroom after reading the result.
...
Read the recommended OCVS sizing totals while retaining the existing cluster structure:
...
Show how one AVS cluster recommendation was determined:
...
python3 scripts/query_rvtools.py WORKBOOK --index INDEX \
  ...
  --select target_cluster --select node_type \
  --select workload_capacity_hosts --select one_host_resilience_hosts \
  --select cloud_service_minimum_hosts --select total_hosts \
  --select what_determined_the_result
```
